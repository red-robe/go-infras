...
func NewMongoClient(appConf *config.AppConfig) *MongoClient {
	var err error
	...
	// 创建连接客户端
	mongoClient.client, err = mongo.NewClient(opt)
	utils.FailHandler(err)

	// 设置默认使用的db
	mongoClient.defaultDb = mongoClient.client.Database(appConf.MongoConf.Database)
