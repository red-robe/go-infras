...
// 创建一个事务
func (m *MysqlClient) NewTx(ctx context.Context, options *sql.TxOptions) (*MysqlTx, error) {
...
*/
func (mtx *MysqlTx) GetMulti(tableName string, where map[string]interface{}, selectField []string, results []DaoMysqlSchema) error {
	if mtx.tx == nil {
		return errors.New("sql.Tx pointer couldn't be nil")
	}
	condition, values, err := builder.BuildSelect(tableName, where, selectField)
